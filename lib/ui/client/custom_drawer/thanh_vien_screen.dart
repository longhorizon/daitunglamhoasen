...
import 'package:admin/ui/client/custom_drawer/fintness_app_theme.dart';
...
class ThanhVienScreen extends StatefulWidget {
  ...
  ThanhVien tv;
  final String url;
  ThanhVienScreen({this.tv, this.url});
  _ThanhVienState createState() => _ThanhVienState();
}

enum GioiTinh { nam, nu }

class _ThanhVienState extends State<ThanhVienScreen> {
  Future<File> file;
  ...
  chonCMND1() {
    setState(() {
      filecmnd1 = ImagePicker.pickImage(source: ImageSource.camera);
    });
  }
  chonCMND2() {
    setState(() {
      filecmnd2 = ImagePicker.pickImage(source: ImageSource.camera);
    });
  }
...
//          print(tmpFile1);
  ...
  _luu() async {
    ...
    if(tmpFile1 !=null){
      startUpload(tmpFile1,'hinhcmnd1');
    }
    if(tmpFile2!=null){
      startUpload(tmpFile2,'hinhcmnd2');
    }
  }
